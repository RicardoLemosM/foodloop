/* =========================================
   FoodLoop - script.js v3 (corrigido)
   ========================================= */

// ===== UTILIDADES =====

function getAlimentos() {
  try { return JSON.parse(localStorage.getItem('foodloop_alimentos')) || []; } catch { return []; }
}
...
function getEstoque() {
  try { return JSON.parse(localStorage.getItem('foodloop_estoque')) || []; } catch { return []; }
}
...
function exibirSaudacao() {
  ...
  if (usuario) {
    ...
    userArea.innerHTML = `<a href="login.html" class="btn-nav-destaque">Entrar / Cadastrar</a>`;
  }
}
...
window.sair = function() {
  ...
  localStorage.removeItem('foodloop_user_local');
  localStorage.removeItem('foodloop_user_email');
...
3. Se não houver correspondências, sugira o que o usuário pode fazer.`
    ...
    if (encontradosLocais.length > 0) {
      ...
      html = `<div class="resultado-ia">🧠 <strong>IA diz:</strong> ${textoIA || 'Não encontrei esse alimento no sistema agora. Que tal verificar mais tarde?'}</div>`;
    }
